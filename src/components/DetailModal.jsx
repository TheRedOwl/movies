...
import { img_500, noPictureLandscape, getData } from '../utils';
...
export const DetailModal=({open,setOpen,id,media_type})=> {
  ...
  const {data:dataVideos,isLoadingVid:isLoadingVideos,isErrorVid:isErrorVideos,error:errorVideos}=useQuery({queryKey:["details",urlVideos],queryFn:getData})

  if(isLoading || isLoadingVideos) return <CircularProgress/>
  if(isError || isErrorVideos) return <div>Error fetching data: {error.message || errorVideos.message}</div>
      ...
      >
        <Box sx={style}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2, display:"flex", flexDirection:"column", alignItems:"center" }}>
            ...
            <span style={{color:"blue"}}>{data?.tagline}</span>
